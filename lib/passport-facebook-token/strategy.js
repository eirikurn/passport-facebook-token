...
function FacebookTokenStrategy(options, verify) {
  ...
  this._appAccessToken = options.appAccessToken;
  this._checkAppID = options.checkAppID !== undefined ? options.checkAppID : true;
...
FacebookTokenStrategy.prototype.authenticate = function(req, options) {
  ...
  self._checkToken(accessToken, function(err, valid) {
    if (err) { return self.error(err); };
    // TODO: Fail with a message indicating app ID mismatch
...
/**
 * Load token info, contingent upon options.
 *
 * @param {String} accessToken
 * @param {Function} done
 * @api private
 */
OAuth2Strategy.prototype._checkToken = function(accessToken, done) {
  if (!this._checkAppID) { return done(null, true); }
  
  var self = this;
  this._oauth2.getProtectedResource('https://graph.facebook.com/debug_token?input_token=' + accessToken, this._appAccessToken, function (err, body, res) {
    if (err) { return done(new InternalOAuthError('failed to fetch token info', err)); }
    
    try {
      var json = JSON.parse(body);
      if (json.data.app_id != self._appID) { return done(null, false); }
      done(null, true);
    } catch(e) {
      done(e);
    }
